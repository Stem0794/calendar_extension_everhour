...
test('popup summary and hours tabs render data', async () => {
  ...
  await expect(
    page.locator('#project-hours-table td', { hasText: 'Project A' }).first()
  ).toBeVisible();
  ...
  await page.screenshot({
    path: path.join(screenshotDir, 'popup-summary-hours.png'),
    fullPage: true
  });
  await browser.close();
});
...
test('project hours per-day filter shows empty message when no events', async () => {
  ...
  await expect(page.locator('#project-hours-table')).toContainText(
    'No project hours for Wednesday',
    { timeout: 6000 }
  );
...
test('summary tab filters meetings by selected day', async () => {
  ...
  await expect(page.locator('#meeting-list td', { hasText: 'Monday Meeting' }).first()).toBeVisible(
    { timeout: 8000 }
  );
  await expect(
    page.locator('#meeting-list td', { hasText: 'Tuesday Meeting' }).first()
  ).toBeVisible();
...
test('assigning a project to a meeting saves it to storage', async () => {
  const events = [{ title: 'Design Review', duration: 60, date: '2023-09-25', dayOfWeek: 1 }];
  ...
  await expect(page.locator('#meeting-list td', { hasText: 'Design Review' }).first()).toBeVisible({
    timeout: 8000
  });
...
test('logs an auto-suggested project without changing the dropdown first', async () => {
  const events = [{ title: 'Design Review', duration: 60, date: '2023-09-25', dayOfWeek: 1 }];
  const projects = [{ name: 'Project X', color: '#00ff00', taskId: 'task-1' }];
  const meetingProjectMap = { 'Design Planning': 'Project X' };
  const { browser, page } = await openPopup(events, projects, meetingProjectMap, {
    everhourToken: 'tok-123'
  });

  const row = page.locator('#meeting-list tr[data-meeting-title="Design Review"]');
  await expect(row.locator('select')).toHaveValue('Project X', { timeout: 8000 });
  await expect(row.locator('.suggest-badge')).toHaveText('suggested');

  let requestBody;
  await page.route('https://api.everhour.com/**', async (route) => {
    requestBody = route.request().postDataJSON();
    await route.fulfill({ status: 200, body: JSON.stringify({ id: 'entry-1' }) });
  });

  await row.locator('.everhour-btn').click();

  await expect(row.locator('.everhour-btn')).toHaveText('✓');
  expect(requestBody).toMatchObject({ task: 'task-1', date: '2023-09-25', time: 3600 });
  const stored = await page.evaluate(() => window.chrome.storage.local._data.meetingProjectMap);
  expect(stored['Design Review']).toBe('Project X');
  await expect(row.locator('.suggest-badge')).toHaveCount(0);
...
test('log all button shows error toast when no Everhour token is set', async () => {
  const events = [{ title: 'Standup', duration: 30, date: '2023-09-25', dayOfWeek: 1 }];
  ...
  await expect(page.locator('#meeting-list td', { hasText: 'Standup' }).first()).toBeVisible({
    timeout: 8000
  });
  ...
  await expect(page.locator('#toast-container')).toContainText(
    'Please set your Everhour token in Settings',
    { timeout: 5000 }
  );
...
test('sync check shows no-token error when token is missing', async () => {
  ...
  await expect(page.locator('#sync-check-status')).toContainText(
    'Set Everhour token in Settings first',
    { timeout: 5000 }
  );
...
test('sync check shows X/Y logged count without making API calls when none logged', async () => {
  ...
  let apiCalled = false;
  await page.route('**/api.everhour.com/**', () => {
    apiCalled = true;
  });
  ...
  await expect(page.locator('#sync-check-status')).toContainText('0/2 entries logged', {
    timeout: 5000
  });
  ...
  await expect(page.locator('#meeting-list tr[data-meeting-title="Standup"]')).toHaveClass(
    /unlogged/
  );
  await expect(page.locator('#meeting-list tr[data-meeting-title="Planning"]')).toHaveClass(
    /unlogged/
  );
...
test('sync check shows correct X/Y when all meetings logged and verified', async () => {
  ...
  await page.route('**/api.everhour.com/time/**', (route) =>
    route.fulfill({ status: 200, body: '{}' })
  );

  await page.click('#sync-check-btn');

  await expect(page.locator('#sync-check-status')).toContainText('Sync OK: 2/2 entries logged', {
    timeout: 5000
  });
